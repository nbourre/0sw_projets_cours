...
func closest_angle_index(action: String, target_deg: float) -> int:
	if not actions.has(action):
		return 0
	return _closest_index(actions[action].angles, target_deg)

func closest_angle_value(action: String, target_deg: float) -> int:
	if not actions.has(action) or actions[action].angles.is_empty():
		return 0
	var idx := closest_angle_index(action, target_deg)
	return int(actions[action].angles[idx])

func animation_name_for(action: String, angle_rad: float, prefix: String = "") -> String:
	if not actions.has(action):
		return ""
	var deg := rad_to_deg(fposmod(angle_rad + angle_offset_rad, TAU))  # 0° = north
	var a := closest_angle_value(action, deg)
	return "%s%s_%03d" % [prefix, action, a]
